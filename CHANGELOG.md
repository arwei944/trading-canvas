...
**前端**
- 全面中文化：6 个组件接入 i18n，62 处硬编码文本替换为 t() 调用
- 默认语言设为中文，新增 16 个翻译键（assets、trend、common 等命名空间）
- 添加缺失的 vite.svg 静态资源

### 修复

**部署**
- 服务器绑定 0.0.0.0 修复 HF Space 连接被拒绝问题
- 移除不存在的 HuggingPlex 字体引用，避免 404 错误

**稳定性**
- API 客户端添加可选链防护（response.data?.data），防止非标准响应导致崩溃
- 所有页面/组件添加 undefined 防御性检查（.length、.map 调用前校验）
- 修复 TagsPage、NotesPage 在 API 不可用时因 undefined 数据崩溃的问题
